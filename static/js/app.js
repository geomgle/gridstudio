...
(function () {
  ...
  var App = function () {
    ...
    this.set_formula = function (position, value, update, sheet) {
      ...
      if (update !== false) {
        this.wsManager.send({
          arguments: [
            "SET",
            this.indexToLetters(position[1] + 1) + (position[0] + 1),
            value.toString(),
            "" + sheet,
          ],
        });
    ...
    this.updateStaticPlotText = function () {
      $(".view.plots .plot-counter").html(
        this.currentStaticPlot + 1 + "/" + this.staticPlotCount,
      );
    };
    ...
    this.showTab = function (selector) {
      $('.dev-tabs .tab[data-tab="' + selector + '"]')
        .addClass("current")
        .siblings()
        .removeClass("current");
    ...
    this.initImagePlotTab = function () {
      ...
      $(".dev-tabs .plots").on("click", "img", function () {
        ...
        $("body").append(
          "<div class='full-plot'><img title='Click to close' src='" +
            image_data +
            "' /></div>",
        );
      });
    ...
    this.getWorkspaceDetails = function () {
      ...
      $(document).on("change", ".workspaceName input", function () {
        ...
        $.post(
          ...
          { workspaceId: id, workspaceNewName: val },
          function (data, error) {
            if (error != "success") {
              console.error(error);
            }
    ...
    this.sortRange = function (direction, range) {
      var rangeString =
        this.cellZeroIndexToString(range[0][0], range[0][1]) +
        ":" +
        this.cellZeroIndexToString(range[1][0], range[1][1]);

      var currentCellLocation = this.positionToCellLocation(
      ...
      _this.wsManager.send({
        arguments: ["SORT", direction, rangeString, column],
      });
    };
      ...
      if (!isNaN(rows) && !isNaN(columns) && rows >= 1 && columns >= 1) {
        ...
        if (rows * columns > 1000000) {
          confirmAmount = confirm(
            "You're creating over a million cells, Grid isn't fully optimized yet. This amount could result in a degraded user experience, are you sure you want to continue?",
          );
        }
    ...
    this.deleteRowColumn = function (type) {
      var clickedCellPosition = _this.positionToCellLocation(
        _this.mouseRightClickLocation[0],
        _this.mouseRightClickLocation[1],
      );
      _this.wsManager.send({
        arguments: [
          type,
          this.cellZeroIndexToString(
            clickedCellPosition[0],
            clickedCellPosition[1],
          ),
        ],
      });
    };
    ...
    this.registerContextMenu = function () {
      $(_this.sheetDom).bind("contextmenu", function (event) {
        ...
        _this.mouseRightClickLocation = [
          event.offsetX - _this.sheetDom.scrollLeft,
          event.offsetY - _this.sheetDom.scrollTop,
        ];
      ...
      $(".context-menu .context-menu-item").click(function (e) {
        ...
        if ($(this).hasClass("sort-asc")) {
          _this.sortRange(
            "ASC",
            _this.selectionToLowerUpper(_this.selectedCells),
          );
          ...
          var selection = _this.cellArrayToStringRange(
            _this.getSelectedCellsInOrder(),
          );
    ...
    this.cutSelection = function () {
      this.cutCopyPasteSelection = new CutCopyPasteSelection(
        this.getSelectedCellsInOrder(),
        this.activeSheet,
        "cut",
      );
    };
    ...
    this.pasteSelection = function () {
      if (this.cutCopyPasteSelection) {
        // send copy command
        var sourceRange = this.cellArrayToStringRange(
          this.cutCopyPasteSelection.cells,
        );
        var destinationRange = this.cellArrayToStringRange(
          this.getSelectedCellsInOrder(),
        );
        ...
        if (this.cutCopyPasteSelection.type == "cut") {
          this.wsManager.send({
            arguments: [
              "CUT",
              sourceRange,
              this.cutCopyPasteSelection.sheetIndex + "",
              destinationRange,
              this.activeSheet + "",
            ],
          });
    ...
    this.setSheets = function (sheetsArray) {
      ...
      for (var x = 0; x < sheetsArray.length; x += 3) {
        ...
        $(".sheet-tabs-holder").append(
          "<div class='sheet-tab'>" + sheetName + "</div>",
        );

        this.data.push([]);
        this.dataFormulas.push([]);
        this.selectedCellsPerSheet.push([
          [0, 0],
          [0, 0],
        ]);
      }

      $(".sheet-tabs-holder .sheet-tab")
        .eq(this.activeSheet)
        .addClass("active");
    ...
    this.switchSheet = function (index) {
      ...
      this.wsManager.send({
        arguments: ["SWITCHSHEET", this.activeSheet + ""],
      });

      this.setSheetSize(
        this.sheetSizes[this.activeSheet][0],
        this.sheetSizes[this.activeSheet][1],
      );
    ...
    this.init = function () {
      ...
      this.isFocusedOnElement = function () {
        ...
        if (
          !focused_on_input &&
          !_this.input_field.is(":focus") &&
          !_this.formula_input.is(":focus") &&
          !_this.editor.ace.isFocused() &&
          !_this.termManager.isFocused()
        ) {
          return false;
        } else {
          return true;
      ...
      this.sheetDom.addEventListener("dblclick", function (e) {
        ...
        if ($(e.target).parents(".plot").length == 0) {
          if (e.which == 1 && !_this.input_field.is(":focus")) {
            ...
            if (
              canvasMouseX < _this.sidebarSize[0] ||
              canvasMouseY < _this.sidebarSize[1]
            ) {
              var type = "column";
              ...
              var cell = _this.positionToCellDivider(
                canvasMouseX,
                canvasMouseY,
                type,
              );
        ...
        if (e.which == 1 && !_this.input_field.is(":focus")) {
          ...
          if (e.target == _this.sheetSizer) {
            ...
            if (
              ...
              var cell = _this.positionToCellLocation(
                canvasMouseX,
                canvasMouseY,
              );
          ...
          if (
            ...
            var clickedCellRef = _this.cellZeroIndexToString(
              clickedCellPosition[0],
              clickedCellPosition[1],
            );
      ...
      this.sheetDom.addEventListener("mousemove", function (e) {
        ...
        if (_this.mouse_down_canvas) {
          if (_this.resizingIndicator) {
            var diff = [
              e.offsetX - _this.resizingIndicatorPosition[0],
              e.offsetY - _this.resizingIndicatorPosition[1],
            ];
            ...
            var cell = _this.positionToCellLocation(
              e.offsetX - _this.sheetDom.scrollLeft,
              e.offsetY - _this.sheetDom.scrollTop,
            );
    ...
    this.cellArrayToStringRange = function (cellRange) {
      var cellIndexStringStart = this.cellZeroIndexToString(
        cellRange[0][0],
        cellRange[0][1],
      );
      var cellIndexStringEnd = this.cellZeroIndexToString(
        cellRange[1][0],
        cellRange[1][1],
      );
    ...
    this.refreshView = function () {
      ...
      var rangeString = this.cellArrayToStringRange([
        [this.drawRowStart, this.drawColumnStart],
        [this.drawRowEnd, this.drawColumnEnd],
      ]);
    ...
    this.refreshDataRange = function (range, sheetIndex) {
      this.wsManager.send(
        '{"arguments":["GET","' + range + '","' + sheetIndex + '"]}',
      );
    };
    ...
    this.show_input_field = function () {
      ...
      if (
        formula &&
        formula[0] == "E" &&
        formula.indexOf("Error in formula:") != -1
      ) {
      ...
      if (
        formula &&
        formula[0] == "C" &&
        formula.indexOf("Circular reference:") != -1
      ) {
      ...
      this.input_field.css({
        marginLeft: cellPosition[0] + 1 + this.sidebarSize[0],
        marginTop: cellPosition[1] + 1 + this.sidebarSize[1],
      });
    ...
    this.deleteSelection = function () {
      ...
      var startCell = this.cellZeroIndexToString(
        lower_upper_cells[0][0],
        lower_upper_cells[0][1],
      );
      var endCell = this.cellZeroIndexToString(
        lower_upper_cells[1][0],
        lower_upper_cells[1][1],
      );
      ...
      for (var r = lower_upper_cells[0][0]; r <= lower_upper_cells[1][0]; r++) {
        ...
        for (
          var c = lower_upper_cells[0][1];
          c <= lower_upper_cells[1][1];
          c++
        ) {
    ...
    this.positionViewOnSelectedCells = function () {
      ...
      if (cellToCenterOn[0] < this.drawRowStart) {
        ...
        var newScrollOffsetY =
          (this.sheetSizer.clientHeight - sheetViewHeight) *
          (cellToCenterOn[0] / this.finalRow);
      ...
      if (cellToCenterOn[1] < this.drawColumnStart) {
        ...
        var newScrollOffsetX =
          (this.sheetSizer.clientWidth - sheetViewWidth) *
          (cellToCenterOn[1] / this.finalColumn);
      ...
      if (cellToCenterOn[1] > viewEndColumn) {
        ...
        var newScrollOffsetX =
          (this.sheetSizer.clientWidth - sheetViewWidth) *
          (minimumFirstColumn / this.finalColumn);
        this.sheetDom.scrollLeft = newScrollOffsetX;
      }
      ...
      if (type == "column") {
        ...
        for (var i = 0; i < this.numColumns; i++) {
          ...
          if (currentColumnWidth >= rowX) {
            ...
            var dist2 = Math.abs(
              rowX - (currentColumnWidth - this.columnWidths(i)),
            );
        ...
        for (var i = 0; i < this.numRows; i++) {
          ...
          if (currentRowHeight >= rowY) {
            ...
            var dist2 = Math.abs(
              rowY - (currentRowHeight - this.rowHeights(i)),
            );
    ...
    this.menuInit = function () {
      ...
      menu.find("menu-item.about").click(function () {
        alert(
          "Grid is a data science environment for the browser. Powered by Python & Go.",
        );
      });
    ...
    this.plot_draggable = function (elem) {
      ...
      $elem.find(".minimize").click(function () {
        ...
        $elem.css({
          transform:
            "translate(" + -oldTransform[0] + "px," + -oldTransform[1] + "px)",
          opacity: 0,
        });
        ...
        setTimeout(function () {
          ...
          _this.menu
            .find('menu-item[data-plot-id="' + plot_id + '"]')
            .removeClass("active");
          $elem.hide();
    ...
    this.get_range_float = function (range, sheetIndex) {
      ...
      return this.get_range(range[0], range[1], sheetIndex).map(
        this.parseFloatForced,
      );
    };
    ...
    this.plot = function (type) {
      ...
      if (type == "scatter") {
        ...
        x_range = [
          [selectedCellsOrdered[0][0], selectedCellsOrdered[0][1]],
          [selectedCellsOrdered[1][0], selectedCellsOrdered[1][1] - 1],
        ];
        y_range = [
          [selectedCellsOrdered[0][0], selectedCellsOrdered[0][1] + 1],
          [selectedCellsOrdered[1][0], selectedCellsOrdered[1][1]],
        ];
      ...
      var plot_div = $(
        '<div class="plot scatter"><div class="resizer"></div><div class="plot-header"><div class="close"><img src="image/cross.svg" /></div><div class="minimize"><img src="image/dash.svg" /></div><div class="save-svg"><img src="image/floppy.svg" /></div></div><div class="plotly-holder" id="' +
          plot_id +
          '" ></div></div>',
      );
      ...
      plot_div
        .find("#" + plot_id)
        .css({ width: plotWidth, height: plotHeight });
    ...
    this.addPlotToMenu = function (plot_id) {
      ...
      menuList.append(
        "<menu-item class='plot-item active' data-plot-id='" +
          plot_id +
          "'>Plot " +
          this.plot_count +
          "</menu-item>",
    ...
    this.drawSheet = function () {
      ...
      var columnPercentage =
        this.scrollOffsetX / (this.sheetSizer.clientWidth - width);
      if (isNaN(columnPercentage)) {
        columnPercentage = 0;
      }
      var rowPercentage =
        this.scrollOffsetY / (this.sheetSizer.clientHeight - height);
      ...
      while (true) {
        if (
          currentY > height + this.rowHeights(drawRowEnd) ||
          drawRowEnd > this.numRows
        ) {
          break;
        }
      ...
      while (true) {
        ...
        this.ctx.a_moveTo(
          0,
          currentY + firstCellHeightOffset + this.sidebarSize[1],
        );
        this.ctx.a_lineTo(
          horizontalLineEndX,
          currentY + firstCellHeightOffset + this.sidebarSize[1],
      ...
      while (true) {
        ...
        this.ctx.a_moveTo(
          currentX + firstCellWidthOffset + this.sidebarSize[0],
          0,
        );
        this.ctx.a_lineTo(
          currentX + firstCellWidthOffset + this.sidebarSize[0],
      ...
      this.renderCells(
        ...
        i - 1,
        d - 1,
        firstCellHeightOffset,
        firstCellWidthOffset,
      );
    ...
    this.updateInputFormula = function () {
      this.formula_input.val(
        this.get_formula(this.selectedCells[0], this.activeSheet),
      );
    };
      ...
      if (this.selectedCells) {
        ...
        if (cell_position) {
          ...
          if (cell_1) {
            ...
            if (
              strokeY > this.sidebarSize[1] &&
              strokeX > this.sidebarSize[0]
            ) {
              this.ctx.strokeRect(
                strokeX,
                strokeY,
          ...
          var cellRangeSize = this.cellRangeDistance(
            cellsForSelected[0],
            cellsForSelected[1],
          );
          ...
          this.ctx.fillRect(0, drawY, this.sidebarSize[0], drawHeight);

          this.ctx.fillRect(drawX, 0, drawWidth, this.sidebarSize[1]);
        }
      }
    };
    ...
    this.renderCells = function (
      ...
      for (var i = startRow; i < endRow; i++) {
        for (var d = startColumn; d < endColumn; d++) {
          ...
          var centeringOffset =
            (this.rowHeights(i) + 2 - this.fontHeight) / 2 + 1;
          ...
          if (cell_data !== undefined && cell_data.length > 0) {
            ...
            var fitted_cell_data = this.fittingStringFast(
              cell_data,
              cellMaxWidth,
            );
            this.ctx.fillText(
          ...
          if (i == startRow) {
            ...
            var centeringOffset =
              (this.sidebarSize[1] + 2 - this.fontHeight) / 2 + 1;

            this.ctx.fillText(
              this.indexToLetters(d + 1),
              currentX +
                firstCellWidthOffset +
                this.sidebarSize[0] +
                centerOffset,
        ...
        var centeringOffset =
          (this.rowHeights(i) + 2 - (this.fontHeight - 3)) / 2 + 1;
        this.ctx.font = "9px Arial";
        this.ctx.fillText(
          i + 1,
          firstCellWidthOffset + centerOffset,
          currentY +
            firstCellHeightOffset +
            this.sidebarSize[1] +
            centeringOffset,
