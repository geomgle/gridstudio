(function () {
  function TestManager(app) {
    ...
    this.reportTestingResult = function () {
      console.log(
        "Testing result: " +
          (this.tests.length - this.testFailCount) +
          "/" +
          this.tests.length +
          " tests succeeded " +
          this.testFailCount +
        ...
        this.currentTestCallback = function () {
          ...
          for (var prop in currentTest.assert) {
            if (currentTest.assert.hasOwnProperty(prop)) {
              ...
              var cellPosition =
                this.app.referenceToZeroIndexedArray(reference);
    ...
    this.testStore["testCopyValue"] = (function () {
      var run = function () {
        ...
        this.app.wsManager.send({
          arguments: ["COPY", "A1:A1", "0", "B1:B1", "0"],
        });
      };
    ...
    this.testStore["testSumAndCut"] = (function () {
      var run = function () {
        ...
        this.app.wsManager.send({
          arguments: ["SET", "B1", "=SUM(A1:A2)", "0"],
        });
        this.app.wsManager.send({
          arguments: ["CUT", "A1:A2", "0", "A3:A4", "0"],
        });
      };
